...
if(n!=3):
    ...
    try:
        ...
        time.sleep(1)
        # opening all servers and storing pids for to close them when needed
        pid = os.fork()
        if(pid==0):
        ...
        if(pid==0):
            ...
            childpids.append(pid)
        # sleep for 1 sec so that client should not open befor server does
        time.sleep(1)
        while(True):
            ...
            if(domainName=="bye"):
                ...
                if(data.decode().strip()=="ERROR 404"):
                    print("No DNS Record Found")
                else:

                    print('DNS Mapping : ', data.decode())
                # clientSocket.close()
